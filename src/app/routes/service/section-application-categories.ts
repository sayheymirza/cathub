  ...
  template: `
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
      @for (item of categories; track $index) {
        <a routerLink="{{item.link}}" class="flex flex-col items-center gap-1 overflow-hidden rounded-2xl relative aspect-square p-8 cursor-pointer group bg-base-200 text-base-content">
          <!-- <div class="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-black to-black/0"></div> -->
            ...
            <span class="opacity-70 w-full z-1">ساخت اپلیکیشن مناسب</span>
            <strong class="w-full text-lg z-1">{{item.title}}</strong>
