...
@Component({
  ...
  template: `
    <form [formGroup]="form" class="md:grid md:grid-cols-2 gap-4">
      ...
      <fieldset class="fieldset col-span-2">
        ...
        <input dir="ltr" formControlName="email" type="email" class="input focus:input-primary w-full" />
      </fieldset>
      ...
      @if(!readonly) {
        ...
        <label class="flex flex-nowrap items-center gap-4 cursor-pointer col-span-2">
          <input [(ngModel)]="confirmed" [ngModelOptions]="{'standalone': true}" type="checkbox" class="checkbox checked:checkbox-primary" />
          <span class="text-sm md:text-base">اینجانب صحت اطلاعات وارد شده در این فرم را تایید می‌نمایم.</span>
        </label>
        
        <button (click)="onSubmit()" [disabled]="form.disabled || !confirmed()" class="btn btn-primary mt-8 md:mt-2 w-fit">
          ثبت درخواست
        </button>
      }
    </form>
